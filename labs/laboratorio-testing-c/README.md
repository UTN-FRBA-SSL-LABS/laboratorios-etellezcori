...
> R:carrito_init inicializa el carrito, dejándolo preparado para ser utilizado y con una cantidad inicial de 0 productos. Es importante llamarla antes de usarlo porque, de lo contrario, el carrito podría contener valores no inicializados y las demás funciones podrían comportarse incorrectamente.
...
> R:Falla. El 5.º carrito_agregar devuelve 1, cuando debería devolver 0.
...
> R:Sí. Las líneas 29 y 30 de carrito.c, correspondientes a carrito_descuento(), aparecen con ##### porque los tests ejecutados para la cobertura son solamente los tests unitarios y ninguno llama a carrito_descuento().
...
> R:Un test unitario prueba una función de manera aislada, mientras que un test de integración verifica que varias funciones trabajen correctamente en conjunto. El bug de carrito_total fue detectado primero por el test unitario test_total_con_cantidad.
...
> R:En C los arrays no realizan comprobaciones automáticas de límites. Si se accede fuera del array, se puede escribir en una zona de memoria incorrecta, provocando corrupción de memoria, errores inesperados o vulnerabilidades. En Python y Java se comprueban los límites y se produce una excepción en lugar de escribir directamente fuera del array.
...
> R:Los tests permiten comprobar automáticamente el comportamiento real del programa y detectar casos en los que el resultado no es el esperado. Además, permiten comprobar que una corrección realmente soluciona el problema y que no rompe otras partes.
...
> R:Porque con cantidad 1, multiplicar por la cantidad o no hacerlo da el mismo resultado: 350 * 1 = 350. Al usar cantidad 2, el resultado correcto es 350 * 2 = 700, por lo que el bug se hace visible.
